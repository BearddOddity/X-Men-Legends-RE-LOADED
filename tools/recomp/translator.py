...
class FunctionTranslator:
    ...
    def translate_function(self, func_addr, func_info):
        ...
        # fnstsw/fstsw belong here too: they now build a real status word out
        # of _fpu_cmp so that the 'fnstsw ax; test ah, 0x41' idiom reads
        # something meaningful, and a function can carry fnstsw without a
        # compare in the same body.
        has_fpu_cmp = any(insn.mnemonic in ("fcompi", "fcomip", "fucomi",
                                             "fucompi", "fucomip", "fcomi",
                                             "fcom", "fcomp", "fcompp",
                                             "fucom", "fucomp", "fucompp",
                                             "fnstsw", "fstsw", "ftst", "fxam")
                          for insn in instructions)
        if has_fpu_cmp:
        ...
        # FPU stack.
        #
        # These alias the GLOBAL x87 stack, not function locals. On x86 the
        # x87 stack is a machine resource, not per-frame: a function returning
        # float or double leaves the value in st(0) and the caller pops it
        # with fstp. When _fp_stack/_fp_top were locals, every such return
        # value was destroyed at the call boundary and the caller popped an
        # uninitialised slot. Same class of bug as ebp, same fix as g_seh_ebp.
        #
        # g_fp_top starts at 0, matching TOP after finit; push predecrements,
        # so the first push lands in slot 7 and wraps downward like hardware.
        if has_fpu:
            ...
            lines.append(f"    #define fp_st(n) g_fp_stack[(g_fp_top + (n)) & 7]")
    ...
    def translate_batch_split(self, func_list, output_dir, chunk_size=1000,
        ...
        if unresolved:
            ...
            stub_lines = [
                ...
                " *",
                " * Each body pops the fake return address and nothing else.",
                " *",
                " * These stubs are NOT harmless. Every call site pushes a fake return",
                " * address that the callee is responsible for popping, so a stub with",
                " * an empty body leaks 4 bytes of simulated stack per call - and any",
                " * argument purge the real function owed (`ret N`) leaks too. Worse,",
                " * when a stub terminates one of the lifter's tail-call chains it also",
                " * swallows the PUSH32/POP32 pairs that restore ebx/esi/edi across the",
                " * chain, so the caller silently continues with someone else's values.",
                " *",
                " * That is not hypothetical: sub_001A0189 was such a stub, and its",
                " * missing 16-byte purge left __SEH_epilog restoring registers from 16",
                " * bytes off-target. _initterm lost its table cursor and started",
                " * calling dwords straight off the stack.",
                " *",
                " * Popping the return address is the floor, not a fix. The real repair",
                " * is to seed them:",
                " *     py -3 tools_data/seed_missing_functions.py --stubs --apply",
                " */",
                "",
                "#define RECOMP_GENERATED_CODE",
                f'#include "{header_name}"',
