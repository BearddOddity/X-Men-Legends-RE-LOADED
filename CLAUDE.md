...
| `fix_decloop_stubs.py` | the **table-fill truncation** class: the lifter emits a returning stub for a `dec %eax` that is only reachable by a short `jmp`, so the loop never finishes and the caller's pops never run. 96 sites, all repaired (#222). Requires the fall-through target to be a lifted function; `--only` stages one site. Verify new ones with `oracle.py --disas` before widening |
| `find_stub_continuations.py` | which unresolved stubs are reached by a **tail jump**? The lifter emits `{ g_esp += 4; }` for undetected addresses, which simulates a `ret`; when the original is a `jmp` the caller's whole tail is skipped and its pushes never pop. 2,057 of 2,122 stubs are in that position. Confirm each with `oracle.py --disas` and transcribe - never guess (#219) |
| `oracle.py` | ask the REAL game, running in xemu, what it does here. `--at ADDR --dump edi,[edi]` for values, `--disas` for the original instructions, `--depth a,b,c` to compare frame sizes against ours. Compare deltas and call order, never absolute addresses - the real heap is at 0x0080xxxx and ours at 0x0109xxxx. One `--depth` run cleared a whole call chain that would have cost one build per function (#216) |
| `fix_stale_flags.py` | repair deferred-flag miscompiles: the lifter emits `cmp` as a comment and re-tests inside the following `jcc`, so anything writing an operand in between makes the branch read the wrong value. Regeneration reintroduces the whole class, so it is a **pipeline step in `phase.py`**, not a one-off. `find_stale_flag_tests.py` is the detector. Repairing 466 sites took reached from 52/205 to 143/433 (#210, #211) |
