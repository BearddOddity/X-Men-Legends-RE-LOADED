@echo off
REM Configure and build the guest page-zero census build.
REM
...
REM
REM Output goes to build_page0/, separate from build/, the same way
...
%CMAKE% -S . -B build_page0 -G Ninja -DCMAKE_BUILD_TYPE=Release -DRECOMP_TRAP_PAGE_ZERO=ON || exit /b 1
%CMAKE% --build build_page0 --config Release || exit /b 1
